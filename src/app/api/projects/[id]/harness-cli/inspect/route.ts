...
import { inspectHarnessRun } from '@studio-core/harness-cli';
...
export async function POST(
  ...
  try {
    ...
    const inspectRequest: HarnessCliInspectRequest = {
      threadId,
      runRoot: readOptionalString(body.runRoot),
    };
    const result = await inspectHarnessRun(project.path, inspectRequest);
    return NextResponse.json({ success: true, data: result });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Failed to inspect harness run';
